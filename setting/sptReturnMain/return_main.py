...
with open("setting/sptReturnMain/config/python_logging_configuration.json", 'r') as configuration_file:
...
def return_main(data):
    """
     data is dictionary {"jid":,"host"}
     host is ip thomson
    """
    jid = data["jid"]
    ...
    if job_status.upper() == "OK":
        ...
        if enable_backup.upper() == "NOTOK":
            ...
            time.sleep(1)
            enable_backup = active_backup(jd)
            logger.warning("Job(%s) enable backup --> %s"%(str(jid), enable_backup))
            time.sleep(1)
            start = start_job(jd)
